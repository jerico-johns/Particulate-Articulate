...
def randomDaySplit(df):
    '''Takes list of dataframes by location and returns the train/val/split for each.'''
    train_split, val_split, test_split = df.select("date").distinct().randomSplit(weights=[0.8, 0.1, 0.1], seed = 0)
    ...
    tabnet_df = df.join(sets, on = "date", how = "left")
    tabnet_df = tabnet_df.drop(*['aod_lon_lat_list', 'pm25_date_d', 'pm25_datetime_dt', 'rank', 'aod_reading_end','pm25_reading_date', 'tz'])
    return tabnet_df

# COMMAND ----------

if split_strategy == 'random_day':
    # Create different dfs for each location. 
    la_df = aod_gfs_joined_with_labels.where("location = 'Los Angeles (SoCAB)'")
    nd_df = aod_gfs_joined_with_labels.where("location = 'Delhi'")
    tp_df = aod_gfs_joined_with_labels.where("location = 'Taipei'")
    la_df_tabnet = randomDaySplit(la_df)
    nd_df_tabnet = randomDaySplit(nd_df)
    tp_df_tabnet = randomDaySplit(tp_df)
    


# COMMAND ----------

display(nd_df_tabnet)
...
la_train = la_df_tabnet.toPandas()
nd_train = nd_df_tabnet.toPandas()
tp_train = tp_df_tabnet.toPandas()

la_train_indices = la_train[la_train.Set=="train"].index
la_valid_indices = la_train[la_train.Set=="valid"].index
la_test_indices = la_train[la_train.Set=="test"].index

nd_train_indices = nd_train[nd_train.Set=="train"].index
nd_valid_indices = nd_train[nd_train.Set=="valid"].index
nd_test_indices = nd_train[nd_train.Set=="test"].index

tp_train_indices = tp_train[tp_train.Set=="train"].index
tp_valid_indices = tp_train[tp_train.Set=="valid"].index
tp_test_indices = tp_train[tp_train.Set=="test"].index
...
la_train, la_categorical_columns, la_categorical_dims = imputeFeatures(la_train, la_train_indices)
nd_train, nd_categorical_columns, nd_categorical_dims = imputeFeatures(nd_train, nd_train_indices)
tp_train, tp_categorical_columns, tp_categorical_dims = imputeFeatures(tp_train, tp_train_indices)

# COMMAND ----------

# Since feature names are same for all dfs we can just run on la_train: 

unused_feat = ['Set', 'rank']

features = [ col for col in la_train.columns if col not in unused_feat+[target]] 

cat_idxs = [ i for i, f in enumerate(features) if f in la_categorical_columns]

cat_dims = [ la_categorical_dims[f] for i, f in enumerate(features) if f in la_categorical_columns]
...
#LA 
X_train_la = la_train[features].values[la_train_indices]
y_train_la = la_train[target].values[la_train_indices].reshape(-1, 1)

X_valid_la = la_train[features].values[la_valid_indices]
y_valid_la = la_train[target].values[la_valid_indices].reshape(-1, 1)

X_test_la = la_train[features].values[la_test_indices]
y_test_la = la_train[target].values[la_test_indices].reshape(-1, 1)

#ND 
X_train_nd = nd_train[features].values[nd_train_indices]
y_train_nd = nd_train[target].values[nd_train_indices].reshape(-1, 1)

X_valid_nd = nd_train[features].values[nd_valid_indices]
y_valid_nd = nd_train[target].values[nd_valid_indices].reshape(-1, 1)

X_test_nd = nd_train[features].values[nd_test_indices]
y_test_nd = nd_train[target].values[nd_test_indices].reshape(-1, 1)

#TP 
X_train_tp = tp_train[features].values[tp_train_indices]
y_train_tp = tp_train[target].values[tp_train_indices].reshape(-1, 1)

X_valid_tp = tp_train[features].values[tp_valid_indices]
y_valid_tp = tp_train[target].values[tp_valid_indices].reshape(-1, 1)

X_test_tp = tp_train[features].values[tp_test_indices]
y_test_tp = tp_train[target].values[tp_test_indices].reshape(-1, 1)
...
def la_objective(space):
    ...
    evaluation_all = [( X_train_la, y_train_la), ( X_valid_la, y_valid_la)]
    
    clf.fit(X_train_la, y_train_la,
            eval_set=[(X_valid_la, y_valid_la)],
            eval_metric="rmse", 
            early_stopping_rounds=40, 
            verbose = 100)
    

    pred = np.array(clf.predict(X_test_la))
    test_r2 = r2_score(y_pred=pred, y_true=y_test_la)
    print("SCORE:", test_r2)
    return {'loss': -test_r2, 'model': clf, 'status': STATUS_OK }

def nd_objective(space):
    clf=XGBRegressor(
                    max_depth = int(space['max_depth']), 
                    objective = space['objective'], 
                    booster = space['booster'], 
    ...
    evaluation_all = [( X_train_nd, y_train_nd), ( X_valid_nd, y_valid_nd)]
    
    clf.fit(X_train_nd, y_train_nd,
            eval_set=[(X_valid_nd, y_valid_nd)],
            eval_metric="rmse", 
            early_stopping_rounds=40, 
            verbose = 100)
    

    pred = np.array(clf.predict(X_test_nd))
    test_r2 = r2_score(y_pred=pred, y_true=y_test_nd)
    print("SCORE:", test_r2)
    return {'loss': -test_r2, 'model': clf, 'status': STATUS_OK }

def tp_objective(space):
    clf=XGBRegressor(
                    max_depth = int(space['max_depth']), 
                    objective = space['objective'], 
                    booster = space['booster'], 
    ...
    evaluation_all = [( X_train_tp, y_train_tp), ( X_valid_tp, y_valid_tp)]
    
    clf.fit(X_train_tp, y_train_tp,
            eval_set=[(X_valid_tp, y_valid_tp)],
            eval_metric="rmse", 
            early_stopping_rounds=40, 
            verbose = 100)
    

    pred = np.array(clf.predict(X_test_tp))
    test_r2 = r2_score(y_pred=pred, y_true=y_test_tp)
    print("SCORE:", test_r2)
    return {'loss': -test_r2, 'model': clf, 'status': STATUS_OK }
...
la_best_hyperparams = fmin(fn = la_objective,
                        space = space,
                        algo = tpe.suggest,
                        max_evals = 20,
                        trials = la_trials)

print("The best LA hyperparams are : ","\n")
print(la_best_hyperparams)

la_best_model_new = la_trials.results[np.argmin([r['loss'] for r in  la_trials.results])]['model']
...
nd_best_hyperparams = fmin(fn = nd_objective,
                        space = space,
                        algo = tpe.suggest,
                        max_evals = 20,
                        trials = nd_trials)

print("The best ND hyperparams are : ","\n")
print(nd_best_hyperparams)

nd_best_model_new = nd_trials.results[np.argmin([r['loss'] for r in  nd_trials.results])]['model']
...
tp_best_hyperparams = fmin(fn = tp_objective,
                        space = space,
                        algo = tpe.suggest,
                        max_evals = 20,
                        trials = tp_trials)

print("The best TP hyperparams are : ","\n")
print(tp_best_hyperparams)

tp_best_model_new = tp_trials.results[np.argmin([r['loss'] for r in  tp_trials.results])]['model']
...
# Combined. 
la_clf_xgb = la_best_model_new

la_clf_xgb.fit(X_train_la, y_train_la,
        eval_set=[(X_valid_la, y_valid_la)],
        early_stopping_rounds=40,
        verbose=10)

nd_clf_xgb = nd_best_model_new

nd_clf_xgb.fit(X_train_nd, y_train_nd,
        eval_set=[(X_valid_nd, y_valid_nd)],
        early_stopping_rounds=40,
        verbose=10)

tp_clf_xgb = nd_best_model_new

tp_clf_xgb.fit(X_train_tp, y_train_tp,
        eval_set=[(X_valid_tp, y_valid_tp)],
        early_stopping_rounds=40,
        verbose=10)
...
#TRAIN 
la_train_preds = np.array(la_clf_xgb.predict(X_train_la))
nd_train_preds = np.array(nd_clf_xgb.predict(X_train_nd))
tp_train_preds = np.array(tp_clf_xgb.predict(X_train_tp))

y_train_preds = np.concatenate((la_train_preds, nd_train_preds, tp_train_preds))
y_train_true = np.concatenate((y_train_la, y_train_nd, y_train_tp)).reshape(-1)

#VAL
la_val_preds = np.array(la_clf_xgb.predict(X_valid_la))
nd_val_preds = np.array(nd_clf_xgb.predict(X_valid_nd))
tp_val_preds = np.array(tp_clf_xgb.predict(X_valid_tp))

y_valid_preds = np.concatenate((la_val_preds, nd_val_preds, tp_val_preds))
y_valid_true = np.concatenate((y_valid_la, y_valid_nd, y_valid_tp)).reshape(-1)

#TEST
la_test_preds = np.array(la_clf_xgb.predict(X_test_la))
nd_test_preds = np.array(nd_clf_xgb.predict(X_test_nd))
tp_test_preds = np.array(tp_clf_xgb.predict(X_test_tp))

y_test_preds =  np.concatenate((la_test_preds, nd_test_preds, tp_test_preds))
y_test_true = np.concatenate((y_test_la, y_test_nd, y_test_tp)).reshape(-1)


train_r2 = r2_score(y_pred = y_train_preds, y_true = y_train_true)
print(f'Train: {train_r2}')

valid_r2 = r2_score(y_pred = y_valid_preds, y_true = y_valid_true)
print(f'Valid: {valid_r2}')

test_r2 = r2_score(y_pred = y_test_preds, y_true = y_test_true)
...
la_train_preds.reshape(-1,1).shape

# COMMAND ----------

nd_train_preds.shape

# COMMAND ----------

tp_train_preds.shape
...
best_model_new = XGBRegressor(base_score=0.5, booster='dart', colsample_bylevel=0.4195198753747372, colsample_bynode=0.8521435545079946, colsample_bytree=0.8222469360523827, enable_categorical=False, gamma=92.1610045347705, gpu_id=-1, importance_type=None, interaction_constraints='', learning_rate=0.10884869243266887, max_delta_step=9, max_depth=7, min_child_weight=0.4326088157173076, monotone_constraints='()', n_estimators=600, n_jobs=8, num_parallel_tree=1, objective='reg:linear', predictor='auto', random_state=0, reg_alpha=40, reg_lambda=150, scale_pos_weight=1, subsample=0.502269441630567, tree_method='exact', validate_parameters=1, verbosity=None)
...
display(submission_df_sp.select('location', 'value'))
...
submission_df_sp.coalesce(1).write.option("header",True).csv("/mnt/capstone/test/submission_03_19_distance20_aod_filtered_gfs_elev_joined_randomday.csv") 
