...
#     # All AOD. 
    aod_gfs_joined_with_labels_read = spark.read.parquet("dbfs:/mnt/capstone/train/aod_gfs_elev_wlabels_joined.parquet")
    # Clean AOD 
...
if INIT_DATASETS == False and disagg: 
    aod_gfs_joined_with_labels_read = spark.read.parquet("dbfs:/mnt/capstone/train/aod_filtered_gfs_elev_wlabels_joined_disaggregated.parquet")
    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.where('AOD_distance_rank <= 20')
    
    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_QA', aod_gfs_joined_with_labels_read.AOD_QA.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_qa_str', aod_gfs_joined_with_labels_read.AOD_qa_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Cloud_Mask_str', aod_gfs_joined_with_labels_read.AOD_QA_Cloud_Mask_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_QA_LWS_Mask_str', aod_gfs_joined_with_labels_read.AOD_QA_LWS_Mask_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Adj_Mask_str', aod_gfs_joined_with_labels_read.AOD_QA_Adj_Mask_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_Level_str', aod_gfs_joined_with_labels_read.AOD_Level_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('BRF_over_snow_str', aod_gfs_joined_with_labels_read.BRF_over_snow_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('BRF_climatology_str', aod_gfs_joined_with_labels_read.BRF_climatology_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_QA_SC_Mask_str', aod_gfs_joined_with_labels_read.AOD_QA_SC_Mask_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('Algo_init_str', aod_gfs_joined_with_labels_read.Algo_init_str.cast('int'))

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('AOD_distance_to_grid_center', aod_gfs_joined_with_labels_read.AOD_distance_to_grid_center.cast('float'))

    str_cols = [item[0] for item in aod_gfs_joined_with_labels_read.dtypes if item[1].startswith('string')]
    str_cols.append('value')

    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.groupBy(*str_cols).mean()
    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.drop(*['avg(value)'])
...
aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.join(df_misr_train,
                                                      on=[aod_gfs_joined_with_labels_read.grid_id==df_misr_train.grid_id,
                                                        aod_gfs_joined_with_labels_read.datetime==df_misr_train.pm25_reading_date  ],
...
for i in range(len(cols_00)):
    ...
    aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn(colName, avg_cols(sf.array(cols_00[i], cols_06[i],cols_12[i],cols_18[i])))
...
def trailing_features(df, cols):
    ...
    for col in cols:
        df = df.withColumn('trailing1d_'+col,sf.mean(df[col])\
                           .over(Window.partitionBy('grid_id').orderBy(df['datetime'].cast("timestamp").cast("long"))\
...
for col_aod in cols_aod:
    aod_gfs_joined_with_labels_read=aod_gfs_joined_with_labels_read.withColumn(col_aod+'_scaled',aod_scale_udf(aod_gfs_joined_with_labels_read[col_aod]))
...
if FEATURE_ENG_TRAILING: 
    aod_gfs_joined_with_labels_read = trailing_features(aod_gfs_joined_with_labels_read,['avg(avg(Optical_Depth_047))_scaled',
...
aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.replace(-math.inf, np.nan)
aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.replace(math.inf, np.nan)
aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.replace(-np.inf, np.nan)
aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.replace(np.nan, np.nan)
aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.replace(math.nan, np.nan)

# COMMAND ----------

aod_gfs_joined_with_labels_read = aod_gfs_joined_with_labels_read.withColumn('date', sf.to_date('datetime'))

# COMMAND ----------

aod_gfs_joined_with_labels = aod_gfs_joined_with_labels_read.select(*['trailing1d_avg(avg(Optical_Depth_047))_scaled',
'day_of_year',
'month',
'week_of_year',
...
unused_feat = ['Set', 'rank','date','datetime']
...
def get_model_classes():
    model_classes = {}
    model_classes['knn'] = KNeighborsRegressor(algorithm = 'ball_tree', leaf_size = 7.0, metric = 'euclidean', 
    ...
    model_classes['dt'] = DecisionTreeRegressor(max_depth=2, max_features='sqrt', max_leaf_nodes=70, min_samples_leaf=6,
                                                min_weight_fraction_leaf=0.5, splitter='random')
    model_classes['et'] = ExtraTreesRegressor(max_depth=125, max_features = 0.1, min_samples_leaf=45, min_samples_split=30)
    model_classes['xgb'] = XGBRegressor(booster='gblinear', colsample_bylevel=0.7441281484221216, colsample_bynode=0.9952844238980576, 
                                        colsample_bytree=0.43137080263693567, gamma=90.48969617802615, 
                                        learning_rate=0.46462269846762483, max_delta_step=0, max_depth=7, 
                                        min_child_weight=0.15352166677454426, n_estimators=800, objective='reg:squarederror', 
                                        reg_alpha=17.0, reg_lambda=218.0, subsample=0.6196554828692673, 
                                        tree_method='exact', random_state=0)
    model_classes['ada'] = AdaBoostRegressor(learning_rate=0.15305407796950432, n_estimators=200, random_state=1)
    model_classes['gbm'] = GradientBoostingRegressor(ccp_alpha=0.026865992982397335, #criterion='squared_error', 
                                                     learning_rate=0.09860431248503371, max_depth=9.636890537242573, 
                                                     max_features='sqrt', min_impurity_decrease=0.9460572363585461, 
                                                     min_samples_split=0.6000000000000001,n_estimators=800, 
                                                     subsample=0.4643197861064673, random_state=1)
    ...
    model_classes['bag'] = BaggingRegressor(bootstrap=True, bootstrap_features=False, max_features=0.6337454617260814,
                                            max_samples=0.012408138997051066, n_estimators=1000, oob_score=True)
...
unsupervised_model = TabNetPretrainer(
    ...
    optimizer_params=dict(lr=2e-2),
    mask_type='entmax',seed=43# "sparsemax",
#     n_d = 1,
#     n_a = 1
...
tabnet_space = {'n_a': hp.choice('n_a', np.arange(8, 64, dtype=int))}
trials = Trials()
def objective(space):
        ...
        pred = np.array(tabnet.predict(X_train))
        train_r2 = r2_score(y_pred=pred, y_true=y_train)
        pred = np.array(tabnet.predict(X_valid))
        valid_r2 = r2_score(y_pred=pred, y_true=y_valid)
        pred = np.array(tabnet.predict(X_test))
        test_r2 = r2_score(y_pred=pred, y_true=y_test)
#         print("Train Score:", train_r2)
#         print("Valid Score:", valid_r2)
#         print("Test Score:", test_r2)
#         print()
        return {'loss': -(valid_r2- (train_r2 - valid_r2)), 'model': tabnet, 'status': STATUS_OK }
...
print(best_hyperparams)
best_tabnet = trials.results[np.argmin([r['loss'] for r in  trials.results])]['model']
...
#              booster='gbtree',
#              colsample_bylevel=0.4851712669596713,
#              colsample_bynode=0.5960604396571887,
#              colsample_bytree=0.5772301162289328, 
#              enable_categorical=False,
#              gamma=85.09121411155456, 
#              gpu_id=-1, 
#              importance_type=None,
#              interaction_constraints='', 
#              learning_rate=0.11523198769059464,
#              max_delta_step=7, 
#              max_depth=8, 
#              min_child_weight=0.7703304032598982,
#              #missing=nan, 
#              monotone_constraints='()', 
#              n_estimators=1100,
#              n_jobs=8, 
...
test_aod_gfs_joined_with_labels_read = spark.read.parquet("dbfs:/mnt/capstone/test/aod_gfs_elev_joined_disagg.parquet")

submission = test_aod_gfs_joined_with_labels_read.select('datetime', 'grid_id')

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.drop(*['aod_lon_lat_list', 'pm25_date_d', 'pm25_datetime_dt', 'aod_reading_end', 'rank'])
...
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.where('AOD_distance_rank <= 20 or AOD_distance_rank IS NULL')

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA', test_aod_gfs_joined_with_labels_read.AOD_QA.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_qa_str', test_aod_gfs_joined_with_labels_read.AOD_qa_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Cloud_Mask_str', test_aod_gfs_joined_with_labels_read.AOD_QA_Cloud_Mask_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_LWS_Mask_str', test_aod_gfs_joined_with_labels_read.AOD_QA_LWS_Mask_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Adj_Mask_str', test_aod_gfs_joined_with_labels_read.AOD_QA_Adj_Mask_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_Level_str', test_aod_gfs_joined_with_labels_read.AOD_Level_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('BRF_over_snow_str', test_aod_gfs_joined_with_labels_read.BRF_over_snow_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('BRF_climatology_str', test_aod_gfs_joined_with_labels_read.BRF_climatology_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_SC_Mask_str', test_aod_gfs_joined_with_labels_read.AOD_QA_SC_Mask_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('Algo_init_str', test_aod_gfs_joined_with_labels_read.Algo_init_str.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Cloud_Mask', test_aod_gfs_joined_with_labels_read.AOD_QA_Cloud_Mask.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_LWS_Mask', test_aod_gfs_joined_with_labels_read.AOD_QA_LWS_Mask.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Adj_Mask', test_aod_gfs_joined_with_labels_read.AOD_QA_Adj_Mask.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_Level', test_aod_gfs_joined_with_labels_read.AOD_Level.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('BRF_over_snow', test_aod_gfs_joined_with_labels_read.BRF_over_snow.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('BRF_climatology', test_aod_gfs_joined_with_labels_read.BRF_climatology.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_SC_Mask', test_aod_gfs_joined_with_labels_read.AOD_QA_SC_Mask.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('Algo_init', test_aod_gfs_joined_with_labels_read.Algo_init.cast('int'))

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_distance_to_grid_center', test_aod_gfs_joined_with_labels_read.AOD_distance_to_grid_center.cast('float'))

str_cols = [item[0] for item in test_aod_gfs_joined_with_labels_read.dtypes if item[1].startswith('string')]
str_cols.append('value')

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.groupBy(*str_cols).mean()
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.drop(*['avg(value)'])

# COMMAND ----------

df_misr_test = spark.read.parquet("/mnt/capstone/test/all_misr.parquet").drop(*['Month','Day','Year','Day_Of_Year','Hour','Minute'])
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.join(df_misr_test,
                                                      on=[test_aod_gfs_joined_with_labels_read.grid_id==df_misr_test.grid_id,
                                                        test_aod_gfs_joined_with_labels_read.datetime==df_misr_test.pm25_reading_date  ],
...
cols_aod = [col for col in test_aod_gfs_joined_with_labels_read.columns if '_047' in col or '_055' in col]
cols_aod = cols_aod + ['Aerosol_Optical_Depth','Absorption_Aerosol_Optical_Depth','Nonspherical_Aerosol_Optical_Depth',
                 'Small_Mode_Aerosol_Optical_Depth','Medium_Mode_Aerosol_Optical_Depth','Large_Mode_Aerosol_Optical_Depth']
for col_aod in cols_aod:
    test_aod_gfs_joined_with_labels_read=test_aod_gfs_joined_with_labels_read.withColumn(col_aod+'_scaled',
                                                                         aod_scale_udf(test_aod_gfs_joined_with_labels_read[col_aod]))
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.drop(*cols_aod)

# COMMAND ----------

if FEATURE_ENG_TRAILING: 
    test_aod_gfs_joined_with_labels_read = trailing_features(test_aod_gfs_joined_with_labels_read,['avg(avg(Optical_Depth_047))_scaled',
                                                                               'avg(avg(Optical_Depth_055))_scaled'])
...
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Cloud_Mask', test_aod_gfs_joined_with_labels_read['avg(AOD_QA_Cloud_Mask)'].cast('string')).drop(*['avg(AOD_QA_Cloud_Mask)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_LWS_Mask', test_aod_gfs_joined_with_labels_read['avg(AOD_QA_LWS_Mask)'].cast('string')).drop(*['avg(AOD_QA_LWS_Mask)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_Adj_Mask', test_aod_gfs_joined_with_labels_read['avg(AOD_QA_Adj_Mask)'].cast('string')).drop(*['avg(AOD_QA_Adj_Mask)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_Level', test_aod_gfs_joined_with_labels_read['avg(AOD_Level)'].cast('string')).drop(*['avg(AOD_Level)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('BRF_over_snow', test_aod_gfs_joined_with_labels_read['avg(BRF_over_snow)'].cast('string')).drop(*['avg(BRF_over_snow)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('BRF_climatology', test_aod_gfs_joined_with_labels_read['avg(BRF_climatology)'].cast('string')).drop(*['avg(BRF_climatology)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('AOD_QA_SC_Mask', test_aod_gfs_joined_with_labels_read['avg(AOD_QA_SC_Mask)'].cast('string')).drop(*['avg(AOD_QA_SC_Mask)'])

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('Algo_init', test_aod_gfs_joined_with_labels_read['avg(Algo_init)'].cast('string')).drop(*['avg(Algo_init)'])

# COMMAND ----------

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.replace(-math.inf, np.nan)
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.replace(math.inf, np.nan)
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.replace(-np.inf, np.nan)
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.replace(np.nan, np.nan)
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.replace(math.nan, np.nan)

# COMMAND ----------

test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('date', sf.to_date('datetime'))

# COMMAND ----------

test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels_read.select(*['trailing1d_avg(avg(Optical_Depth_047))_scaled',
'day_of_year',
'month',
'week_of_year',
'Aerosol_Optical_Depth_scaled',
...
test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels.drop(*['datetime','grid_id'])
...
preds = np.array(tabnet.predict(X_test))
...
show_df = pd.DataFrame(X_test, columns = features)
show_df['value'] = preds
show_df
...
submission_df_sp.coalesce(1).write.option("header",True).mode("overwrite").csv("/mnt/capstone/test/submission_03_21_featsel_tn_disagg_dist20_randomday.csv") 
