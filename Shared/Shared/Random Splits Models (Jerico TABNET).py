...
if FEATURE_ENG_TRAILING: 
    aod_gfs_joined_with_labels_read = trailing_features(aod_gfs_joined_with_labels_read,['min_Optical_Depth_047_scaled',
...
if split_strategy == 'random_day':
    train_split, val_split, test_split = aod_gfs_joined_with_labels.select("pm25_date_d").distinct().randomSplit(weights=[0.8, 0.1, 0.1], seed = 0)
...
unused_feat = ['Set', 'rank','pm25_date_d']
...
test_aod_gfs_joined_with_labels_read.columns
...
test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels_read.select(*['trailing1d_min_Optical_Depth_047_scaled',
                                                                                'trailing1d_median_Optical_Depth_047_scaled',
                                                                      'trailing1d_max_Optical_Depth_047_scaled',
                                                                      'trailing1d_median_Optical_Depth_055_scaled',
                                                                        'day_of_year',
                                                                        'month',
                                                                        'week_of_year',
                                                                        'Aerosol_Optical_Depth_scaled',
...
unused_feat = ['Set', 'datetime', 'grid_id']
...
submission_df_sp.coalesce(1).write.option("header",True).mode("overwrite").csv("/mnt/capstone/test/submission_03_21_featsel_ens_agg_allaod_total_random.csv") 
