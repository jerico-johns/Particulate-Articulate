...
avg_cols = sf.udf(lambda array: sum(array)/len(array), DoubleType())

# COMMAND ----------

for i in range(len(cols_00)):
    colName = cols_00[i] + "_merged"
    aod_gfs_joined_with_labels = aod_gfs_joined_with_labels.withColumn(colName, avg_cols(sf.array(aod_gfs_joined_with_labels[cols_00[i]], aod_gfs_joined_with_labels[cols_06[i]], aod_gfs_joined_with_labels[cols_12[i]],
                                                                                             aod_gfs_joined_with_labels[cols_18[i]])))
...
aod_gfs_joined_with_labels = aod_gfs_joined_with_labels.where('AOD_distance_rank = 1')
# aod_gfs_joined_with_labels_5 = aod_gfs_joined_with_labels.where('AOD_distance_rank <=5')

# str_cols = [item[0] for item in aod_gfs_joined_with_labels_5.dtypes if item[1].startswith('string')]
            
# str_cols.append('value')
# str_cols.append('AOD_distance_rank')

# aod_gfs_joined_with_labels = aod_gfs_joined_with_labels_5.groupBy(*str_cols).mean()
...
clf_xgb = XGBRegressor(base_score=0.5, booster='gbtree', colsample_bylevel=0.7188400815035519, colsample_bynode=0.9763449696017316, colsample_bytree=0.49002216806115917, enable_categorical=False, gamma=20.78926416269247, gpu_id=-1, importance_type=None, interaction_constraints='', learning_rate=0.5024687297306211, max_delta_step=10, max_depth=9, min_child_weight=0.5302457723691564, monotone_constraints='()', n_estimators=700, n_jobs=8, num_parallel_tree=1, objective='reg:linear', predictor='auto', random_state=0, reg_alpha=78, reg_lambda=153, scale_pos_weight=1, subsample=0.7219404175332462, tree_method='exact', validate_parameters=1, verbosity=None)
...
test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels.where('AOD_distance_rank = 1 or AOD_distance_rank IS NULL')
test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels.withColumn('date', to_date('datetime'))

# test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels.withColumn('AOD_distance_to_grid_center', test_aod_gfs_joined_with_labels.AOD_distance_to_grid_center.cast('int'))
...
test_aod_gfs_joined_with_labels.select("grid_id", "datetime").distinct().count()
...
# test_aod_gfs_joined_with_labels = drop_GFS(test_aod_gfs_joined_with_labels, rename_list)
...
submission_df_sp.coalesce(1).write.option("header",True).csv("/mnt/capstone/test/submission_03_19_distance1_aod_filtered_gfs_elev_joined_randomday.csv") 
