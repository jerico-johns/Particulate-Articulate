...
anvil.server.connect("server_CEQEQ4J3HKTSIEPERNW7WEJW-IITUKUZISZRCO3KS")
...
aod_gfs_joined_with_labels_read.where(aod_gfs_joined_with_labels_read['value'] < 0).count()

# COMMAND ----------

aod_gfs_joined_with_labels_read.where(aod_gfs_joined_with_labels_read['value'] > 600).count()

# COMMAND ----------

display(aod_gfs_joined_with_labels_read.where('t_surface00 > 0'))
...
aod_gfs_joined_with_labels_train = aod_gfs_joined_with_labels.where("date_utc < '2021-04-01'")
...
#                                                .otherwise("test")).cache()
    ...
    tabnet_df = tabnet_df.drop(*['datetime_utc','date_utc'])
...
if split_strategy=='random':
    tabnet_df = aod_gfs_joined_with_labels_train
    tabnet_df = tabnet_df.drop(*['datetime_utc','date_utc'])
#                                ['aod_lon_lat_list', 'pm25_date_d', 'pm25_datetime_dt', 'rank', 'aod_reading_end',
#                                  'datetime','pm25_reading_date', 'tz'])
    train = tabnet_df.toPandas()
...
unused_feat = ['Set', 'rank', 'grid_id', 'location', 'parameter', 'difflag']+lag_feats
categorical_dims =  {}
for col in [c for c in train.columns[train.dtypes == object] if c not in unused_feat]:
...
space={'max_depth': hp.quniform("max_depth", 3, 18, 1),
       'objective': hp.choice('objective', ['reg:squarederror', 'reg:linear']), 
       'booster': hp.choice('booster', ['gbtree']), #'gblinear' and 'dart' doesnt work with shap for explainability
       'max_delta_step': hp.quniform('max_delta_step', 0, 10, 1),
       'subsample': hp.uniform('subsample', 0, 1), 
...
shap_values = shap.TreeExplainer(best_model_new).shap_values(train[features])
...
# test_aod_gfs_joined_with_labels_read = spark.read.parquet("dbfs:/mnt/capstone/test/test_aod_gfs_elev_misr_wlabels.parquet")
# submission = test_aod_gfs_joined_with_labels_read.select('datetime', 'grid_id')
# test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.drop(*['aod_lon_lat_list', 'pm25_date_d', 'pm25_datetime_dt', 'aod_reading_end','misr_reading_end', 'rank'])
...
test_full = aod_gfs_joined_with_labels_test.toPandas()
features_clean = [ col for col in test_full.columns ] 
test_clean = test_full[features_clean].values
# aod_gfs_joined_with_labels_test = aod_gfs_joined_with_labels_test.drop(*['date_utc'])
    
aod_gfs_joined_with_labels_test = aod_gfs_joined_with_labels_test.drop(*['datetime_utc','date_utc'])#,'grid_id','difflag', 'location', 'parameter'])

# COMMAND ----------

train = tabnet_df.toPandas()
test = aod_gfs_joined_with_labels_test.toPandas()
target = 'value'
...
best_model_new = XGBRegressor(base_score=0.5, 
             booster='gbtree',
             colsample_bylevel=0.4851712669596713,
             colsample_bynode=0.5960604396571887,
             colsample_bytree=0.5772301162289328, 
...
train_preds=np.array(best_model_new.predict(X_train))
final_train_r2 = r2_score(y_pred=train_preds, y_true=y_train)
print(final_train_r2)
final_train_mse = mean_squared_error(y_pred=train_preds, y_true=y_train)
...
final_test_r2 = r2_score(y_pred=preds, y_true=y_test)
print(final_test_r2)
final_test_mse = mean_squared_error(y_pred=preds, y_true=y_test)
...
submission_df = df[['latitude', 'longitude', 'date_utc', 'pred_value']]
submission_df = submission_df.sort_values(by = ['date_utc', 'latitude', 'longitude'])
...
def generate_pred(lat, lon):
    try:
        ...
        pred_value = df_maxd['pred_value'].iloc[0]
        return pred_value
    except:
...
generate_pred(0.830586,-117.938509)
...
submission_df_sp.coalesce(1).write.option("header",True).mode("overwrite").csv("/mnt/capstone/test/submission_0408_random_split_new.csv") 
