...
#     # All AOD. 
    aod_gfs_joined_with_labels = spark.read.parquet("dbfs:/mnt/capstone/train/aod_gfs_geo_joined_with_labels.parquet")
    # Clean AOD 
...
tabnet_df = tabnet_df.drop(*['aod_lon_lat_list', 'pm25_date_d', 'pm25_datetime_dt', 'rank', 'aod_reading_end', 'wkt', 'pm25_reading_date', 'tz', 'avg(co)', 'avg(no2)', 'avg(o3)', 'avg(so2)']) #'avg(pm25_rh35_gcc)', 'avg(co)', 'avg(no2)', 'avg(o3)', 'avg(so2)'
...
unused_feat = ['Set', 'rank', 'geo_datetime', 'geo_grid_id']
...
submission_df_sp.coalesce(1).write.option("header",True).csv("/mnt/capstone/test/submission_03_17_aod_gfs_elev_joined_5_5_8_5_5.csv") 
