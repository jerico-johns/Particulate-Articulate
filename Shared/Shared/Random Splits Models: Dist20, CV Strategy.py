...
def feature_eng_time(df):
    df = df.withColumn('day_of_month', dayofmonth(df.pm25_reading_date))
    df = df.withColumn('day_of_week', dayofweek(df.pm25_reading_date))
    df = df.withColumn('day_of_year', dayofyear(df.pm25_reading_date))
    df = df.withColumn('week_of_year', weekofyear(df.pm25_reading_date))
    df = df.withColumn('month', month(df.pm25_reading_date))
    df = df.withColumn('year', year(df.pm25_reading_date))
    return df
...
#Assert no negative inf
np.any(X_train == -math.inf)
np.any(X_test == -math.inf)
...
def get_model_classes():
    model_classes = {}
    model_classes['knn'] = KNeighborsRegressor(algorithm='auto', leaf_size=10.0, metric='euclidean', n_jobs=-1, 
                                               n_neighbors=12, weights='uniform')
    model_classes['svm'] = SVR() #Notes: Very slow, warnings about running on samples of 10,000+ 
    model_classes['rf'] = RandomForestRegressor(bootstrap=False, max_depth=10.0, 
                                                max_features='auto', min_samples_split=6, n_estimators=600)
    model_classes['dt'] = DecisionTreeRegressor(max_depth=5, max_features='auto', max_leaf_nodes=20, min_samples_leaf=6,
                                                min_weight_fraction_leaf=0.30000000000000004, splitter='random')
    model_classes['et'] = ExtraTreesRegressor(max_depth=100.0, min_samples_leaf=25, min_samples_split=35)
    model_classes['xgb'] = XGBRegressor(booster='gblinear', colsample_bylevel=0.8338557421373465, colsample_bynode=0.4188956697361008, 
                                        colsample_bytree=0.9505357368385932, gamma=91.60197681868308, 
                                        learning_rate=0.41314070459673446, max_delta_step=4, max_depth=12, 
                                        min_child_weight=0.7054519281183445, n_estimators=500, objective='reg:squarederror', 
                                        reg_alpha=36.0, reg_lambda=240.0, subsample=0.7321208929082005, 
                                        tree_method='exact', random_state=0)
    model_classes['ada'] = AdaBoostRegressor(learning_rate=0.00010263525153426043, n_estimators=50, random_state=1)
    model_classes['gbm'] = GradientBoostingRegressor(ccp_alpha=0.009772097759733912, criterion='mse', learning_rate=0.010572590259744274,
                                                     max_depth=8.940917113968815, max_features='auto', 
                                                     min_impurity_decrease=0.4903253118681882, min_samples_split=3,
                                                     n_estimators=500, subsample=0.68386274258302, random_state=1)
    model_classes['lr'] = LinearRegression(normalize=False)
    model_classes['nb'] = GaussianNB() #Notes: not suited for our feature space
    model_classes['nn'] = MLPRegressor(activation='tanh', alpha=0.1665264267040525, beta_1=0.9437180891012691, beta_2=0.9983706527475311,
                                       epsilon=0.0008675215392752822, hidden_layer_sizes=(500, 1000, 500),
                                       learning_rate='adaptive', learning_rate_init=0.061284425608596774, momentum=0.6552790893692165,
                                       shuffle=True, solver='adam')
    model_classes['bag'] = BaggingRegressor(bootstrap=False, bootstrap_features=False, max_features=0.8934496656427924,
                                            max_samples=0.014566940218174618, n_estimators=100, oob_score=False)
...
#         print()
        return {'loss': -test_r2, 'model': model, 'status': STATUS_OK }
...
final_ensemble_model = build_ensemble(X_train, X_valid, X_test, y_train, y_valid, y_test, 
                                      ...
                                      model_types_to_include=['knn', 'rf','dt','et','xgb','ada','gbm','lr','nn','bag'],
                                      mdl_early_stopping_rounds=40, tune_max_evals=40, tune=True)#,
#                    model_types_to_include=['svm','knn','rf','et','xgb','ada','gbm','lr','nb','nn','bag'])
...
clf_xgb = XGBRegressor(base_score=0.5, 
                       booster='gbtree',
...
# best_model_new = XGBRegressor(base_score=0.5, booster='gbtree',
#              colsample_bylevel=0.4572943401260166,
#              colsample_bynode=0.9817802074404147,
#              colsample_bytree=0.385678979854995, enable_categorical=False,
#              gamma=4.263935519930179, gpu_id=-1, importance_type=None,
#              interaction_constraints='', learning_rate=0.1716282906637911,
#              max_delta_step=10, max_depth=11,
#              min_child_weight=0.4733776751261309,
#              monotone_constraints='()', n_estimators=500, n_jobs=8,
#              num_parallel_tree=1, objective='reg:linear', predictor='auto',
#              random_state=0, reg_alpha=25, reg_lambda=111, scale_pos_weight=1,
#              subsample=0.8941730932476202, tree_method='exact',
...
submission_df_sp.coalesce(1).write.option("header",True).mode("overwrite").csv("/mnt/capstone/test/submission_03_19_ens_aod_filtered_gfs_elev_joined_randomday.csv") 
