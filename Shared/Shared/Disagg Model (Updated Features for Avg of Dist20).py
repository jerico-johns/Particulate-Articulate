...
aod_gfs_joined_with_labels = aod_gfs_joined_with_labels.withColumn('date', sf.to_date('datetime'))
...
display(dbutils.fs.ls("/mnt/capstone/test"))
...
features_to_test = ['trailing1d_Optical_Depth_047', 
'day_of_year',
'month', 
'week_of_year', 
...
# test_aod_gfs_joined_with_labels_read_1 = test_aod_gfs_joined_with_labels_read.where('AOD_distance_rank = 1')
...
#Save grouped dataframe for submission 
submission = test_aod_gfs_joined_with_labels_read

# COMMAND ----------

submission = test_aod_gfs_joined_with_labels_read.drop(*['value'])
...
rename_list = ['avg(max(t_surfaceXX_new))',
               'avg(max(pbl_surfaceXX_new))', 
              'avg(max(hindex_surfaceXX_new))', 
               'avg(max(gust_surfaceXX_new))', 
               'avg(max(r_atmosphereXX_new))', 
               'avg(max(pwat_atmosphereXX_new))', 
               'avg(max(u_pblXX_new))', 
               'avg(max(v_pblXX_new))', 
               'avg(max(vrate_pblXX_new))']
          
test_aod_gfs_joined_with_labels_read = rename_GFS(test_aod_gfs_joined_with_labels_read, rename_list)
...
test_aod_gfs_joined_with_labels_read = test_aod_gfs_joined_with_labels_read.withColumn('trailing1d_Optical_Depth_047', sf.mean(test_aod_gfs_joined_with_labels_read['avg(avg(Optical_Depth_047))_scaled']).over(Window.partitionBy('grid_id').orderBy(test_aod_gfs_joined_with_labels_read['datetime'].cast("timestamp").cast("long")).rangeBetween(-days(1), 0)))
...
test_aod_gfs_joined_with_labels = test_aod_gfs_joined_with_labels_read.select(*['trailing1d_Optical_Depth_047', 
'day_of_year',
'month', 
'week_of_year', 
'Aerosol_Optical_Depth_scaled',
...
train_aod_gfs_joined_with_labels = tabnet_df.select(*['trailing1d_Optical_Depth_047', 
'day_of_year',
'month', 
'week_of_year', 
'Aerosol_Optical_Depth_scaled',
...
test_full = test_aod_gfs_joined_with_labels_read.select('trailing1d_Optical_Depth_047', 
'day_of_year',
'month', 
'week_of_year', 
'Aerosol_Optical_Depth_scaled',
...
# features_clean = [ col for col in test_full ] 
# test_full = test_full.toPandas()
# test_clean = test_full[features_clean]
...
train = train_aod_gfs_joined_with_labels.toPandas()
...
best_model_new = XGBRegressor(base_score=0.5, booster='dart', colsample_bylevel=0.7264405802781837, colsample_bynode=0.6717931141747524, colsample_bytree=0.9098057033178444, enable_categorical=False, gamma=104.29312864433498, gpu_id=-1, importance_type=None, interaction_constraints='', learning_rate=0.22010224580941012, max_delta_step=9, max_depth=3, min_child_weight=0.045367779861160806, monotone_constraints='()', n_estimators=1000, n_jobs=8, num_parallel_tree=1, predictor='auto', random_state=0, reg_alpha=57, reg_lambda=177, scale_pos_weight=1, subsample=0.5915472737469036, tree_method='exact', validate_parameters=1, verbosity=None)
...
df = pd.DataFrame(data = test_full, columns = features_clean)

# COMMAND ----------

test_full['value'] = preds
df = test_full
...
submission_df_sp.coalesce(1).write.option("header",True).csv("/mnt/capstone/test/submission_03_21_distance20_aod_filtered_gfs_elev_joined_randomday_2fold.csv") 
